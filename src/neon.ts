...
export const prismaUserCreate = async (user: app_user) =>
  prisma.app_user.create({ data: user });

export const prismaAssignProgressFromBook = async (
  studentId: number,
  bookTitle: string,
) => {
  ...
  const progressArray = resultFindMany.reduce(
    (acc: progress[], questionGroup: { id: number }) => {
  ...
  const resultCreateMany = await prisma.progress.createMany({
    data: progressArray,
  });
  // const book = await
...
export const prismaGetAllProgress = async (studentId: number) => {
  ...
  const groupedProgressArray = Object.groupBy(
    progressArray,
    (progress) => progress.bookTitle,
  );
  console.timeEnd("⏱ group timer");
  return groupedProgressArray;
};

export const prismaPatchProgress = async (
  patchingPropertyName: string,
  editedDict: Record<number, any>,
) => {
...
export const prismaAssignReviewCheckFromBook = async (
  studentId: number,
  bookTitle: string,
) => {
  const student = await prisma.app_user.findUnique({
  ...
  const reviewCheckArray = resultFindMany.reduce(
    (acc: review_check[], question) => {
  ...
  const resultCreateMany = await prisma.review_check.createMany({
    data: reviewCheckArray,
  });
  // const book = await
  return resultCreateMany;
};
...
export const prismaGetAllReviewCheck = async (studentId: number) => {
  ...
  const reiviewCheckGroupedByBook = Object.groupBy(
    reviewCheckArray,
    (reviewCheck) => reviewCheck.bookTitle,
  );
  const entryArrayGroupedByBook = Object.entries(reiviewCheckGroupedByBook);

  const reviewCheckGrouped = entryArrayGroupedByBook.reduce(
    (acc: any, entry) => {
      const array = entry[1];
      // 아휴 진짜 못해먹겠다.

      const groupedByPage = Object.groupBy(
        array!,
        (reviewCheck) => reviewCheck.questionPage,
      );
...
export const prismaPatchReviewCheck = async (
  patchingPropertyName: string,
  editedDict: Record<number, any>,
) => {
  const entryArray = Object.entries(editedDict);
